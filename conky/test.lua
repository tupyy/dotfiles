...
local function uptime_query(node, job)
  return string.format(
    "node_time_seconds{instance=\"%s\",job=\"%s\"} - node_boot_time_seconds{instance=\"%s\",job=\"%s\"}", node, job, node,
    job);
end

local function cpu_query(node, job)
  return string.format(
    "(sum by(instance) (irate(node_cpu_seconds_total{instance=\"%s\",job=\"%s\", mode!=\"idle\"}[10m])) / on(instance) group_left sum by (instance)((irate(node_cpu_seconds_total{instance=\"%s\",job=\"%s\"}[10m])))) * 100",
    node, job, node, job);
end
...
local function rootfs_query(node, job)
  local avail_bytes = string.format(
    "node_filesystem_avail_bytes{instance=\"%s\",job=\"%s\",mountpoint=\"/\",fstype!=\"rootfs\"}", node, job)
  local total = string.format(
    "node_filesystem_size_bytes{instance=\"%s\",job=\"%s\",mountpoint=\"/\",fstype!=\"rootfs\"}", node, job);
  return string.format("100 - ((%s * 100) / %s)", avail_bytes, total);
end

local function datafs_query(node, job)
  local avail_bytes = string.format(
    "node_filesystem_avail_bytes{instance=\"%s\",job=\"%s\",mountpoint=\"/data\",fstype!=\"rootfs\"}", node, job)
  local total = string.format(
    "node_filesystem_size_bytes{instance=\"%s\",job=\"%s\",mountpoint=\"/data\",fstype!=\"rootfs\"}", node, job);
  return string.format("100 - ((%s * 100) / %s)", avail_bytes, total);
end

local function temp_query(node, job)
...
local function count_screens()
  local output = exec("xrandr --listactivemonitors")
...
for _, v in pairs(hosts) do
  ...
  local temp = extract_data(exec(create_curl_req(temp_query(node, job))))
