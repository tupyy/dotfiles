...
local prometheus_url = "https://prometheus.tls.tupangiu.ro/api/v1/query"

local podman_query =
"sum by (name,ports,instance) (podman_container_info*on(id) group_right(name,ports,instance) podman_container_state)";
local curl = "curl -s --data-urlencode 'query=" .. podman_query .. "' " .. prometheus_url;
...
local function format_conky_table(initial_offset, columns, results)
    ...
    for _, r in pairs(results) do
        if (r.name:match("infra") or r.name:match("service")) then goto continue end

        local line = r.state.color;
        line = string.format("%s%s%s", line, format_output(initial_offset, columns_def.name.offset),
            string.sub(r.name, 0, 20));
        line = string.format("%s%s%s", line, format_output(initial_offset, columns_def.host.offset),
            columns_def.host.transform(r.host));
        line = string.format("%s%s%s", line, format_output(initial_offset, columns_def.ports.offset),
            columns_def.ports.transform(r.ports));
